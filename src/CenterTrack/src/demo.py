...
def demo(opt):
  ...
  video_path, video_name = os.path.split(opt.demo)
  out_name = '../results/tracked_{}.avi'.format(video_name.split('.')[0])
  print('out_name:', out_name)
  if opt.save_video:
    # fourcc = cv2.VideoWriter_fourcc(*'XVID')
    # fourcc = cv2.VideoWriter_fourcc(*'H264')
    fourcc = cv2.VideoWriter_fourcc('M','J','P','G')
    out = cv2.VideoWriter(out_name,fourcc, opt.save_framerate, (opt.video_w, opt.video_h))
...
def save_and_exit(opt, out=None, results=None, out_name=''):
  if opt.save_results and (results is not None):
    save_dir =  out_name.replace('.avi', '.json')
    print('saving results to', save_dir)
    json.dump(_to_list(copy.deepcopy(results)), open(save_dir, 'w'), indent=4)
