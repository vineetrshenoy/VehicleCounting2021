...
class TrackerCT():
    ...
    def update_trackers(self, dets, frame_num):
        ...
        active_dets = list(filter(lambda x: x['active'] == 1, dets))
        
        #TODO: Further filter by age: 

        for clsnum, clsname  in zip([3, 6, 8], ['Car', 'Bus', 'Truck']):
            
            class_output = list(filter(lambda x: x['class'] == clsnum, active_dets))
            self.process_tracker_output(class_output, self.frameBox[clsname], 
                self.trackBox[clsname], frame_num - 1)
    ...
    # How often this is run is determined by the detection_tracker module
    #
